...
public class MatriculaDAO extends ConexionBd implements Crud {
    ...
    private String idMatr = "", idAlum = "",alumMatr = "", idUsua = "", regiMatr = "", fechMatr = "", estaMatr = "";
    ...
    public MatriculaVO ConsultarMatricula(String regiMatr) {
        ...
        try {
            ...
            while (mensajero.next()) {
                matVO = new MatriculaVO(mensajero.getString(1), mensajero.getString(2), mensajero.getString(3), mensajero.getString(4), mensajero.getString(5), mensajero.getString(6), mensajero.getString(7));
            }
    ...
    public ArrayList<MatriculaVO> listarEst() {
        ...
        try {
            conexion = this.obtenerConexion();
            sql = "SELECT m.ID_MATR, m.ID_USUA, m.ID_ALUM, m.ALUM_MATR, m.REGIS_MATR, m.FECH_MATR, m.ESTADO_MATR FROM matricula as m WHERE m.ID_USUA IS NULL;";
            puente = conexion.prepareStatement(sql);
            mensajero = puente.executeQuery();
            while (mensajero.next()) {
    ...
    public ArrayList<MatriculaVO> listarInactivas() {

        ArrayList<MatriculaVO> listaMatrEst = new ArrayList<>();

        try {
            conexion = this.obtenerConexion();
            sql = "SELECT m.ID_MATR, u.CORREO_USUA, m.ID_ALUM,m.ALUM_MATR, m.REGIS_MATR, m.FECH_MATR, m.ESTADO_MATR FROM matricula as m INNER JOIN usuario as u ON u.ID_USUA = m.ID_USUA WHERE m.ESTADO_MATR = 'inactivo';";
            puente = conexion.prepareStatement(sql);
            mensajero = puente.executeQuery();
            while (mensajero.next()) {
    ...
    public ArrayList<MatriculaVO> listarActivas() {

        ArrayList<MatriculaVO> listaMatrEst = new ArrayList<>();

        try {
            conexion = this.obtenerConexion();
            sql = "SELECT m.ID_MATR, u.CORREO_USUA, m.ID_ALUM,m.ALUM_MATR, m.REGIS_MATR, m.FECH_MATR, m.ESTADO_MATR FROM matricula as m INNER JOIN usuario as u ON u.ID_USUA = m.ID_USUA WHERE m.ESTADO_MATR = 'activo';";
            puente = conexion.prepareStatement(sql);
            mensajero = puente.executeQuery();
            while (mensajero.next()) {
